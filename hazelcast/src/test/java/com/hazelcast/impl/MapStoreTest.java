...
public class MapStoreTest extends TestUtil {
    ...
    public void testThreeMemberInit() throws Exception {
        ...
        IMap map3 = h3.getMap("default");
        for (int i = 0; i < size; i++) {
            assertEquals("value" + i, map1.get(i));
        }
        System.out.println(testMapStore.callCount.get());
    }

    @Test
    public void testThreeMemberInit2() throws Exception {
        System.setProperty(GroupProperties.PROP_WAIT_SECONDS_BEFORE_JOIN, "1");
        System.setProperty(GroupProperties.PROP_VERSION_CHECK_ENABLED, "false");
        TestEventBasedMapStore testMapStore = new TestEventBasedMapStore();
        testMapStore.setLoadAllKeys(true);
        ...
        Assert.assertTrue(latch.await(100, TimeUnit.SECONDS));
        assertEquals(10000, instances[2].getMap("default").size());
        System.out.println(testMapStore.callCount.get());
    }
    ...
    public void testWriteBehindBackupLoaded() throws Exception {
        ...
        map2.putTransient("2", "value2", 3600 * 24, TimeUnit.SECONDS);
        ...
        assertTrue(record.isValid());
        assertTrue(record.isActive());
        map2.putTransient("2", "value222", 3600 * 24, TimeUnit.SECONDS);
        assertTrue(record.isDirty());
        assertTrue(record.isValid());
        assertTrue(record.isActive());
    }
